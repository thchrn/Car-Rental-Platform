    ...
    <div onClick={() => {navigate(`/car-details/${car._id}`); scrollTo(0,0)}}
      ...
      <div className='p-4 sm:p-5'>
        ...
        <div className='mt-4 grid grid-cols-2 gap-y-2 text-gray-600'>
          ...
          <div className='flex items-center text-sm text-muted-foreground'>
            <img src={assets.location_icon_icon} alt="" className='h-4 mr-2'/>
            <span>{car.location}</span>
          </div>
        </div>

        {(car.owner?.businessName || car.owner?.phone) && (
          <div className='mt-3 pt-3 border-t border-borderColor text-xs text-gray-500 space-y-1'>
            {car.owner?.businessName && (
              <p className='font-medium text-gray-700'>{car.owner.businessName}</p>
            )}
            {car.owner?.phone && (
              <p>{car.owner.phone}</p>
            )}
